...
it('updates modeled fields, regenerates payload, bumps etag, and records a change', function () {
    ...
    $object = DavCalendarObject::factory()->for($calendar, 'calendar')->state(davData(['summary' => 'Old']))->create()->fresh();
    ...
    expect($updated->data->summary)->toBe('New title')
...
it('preserves unmodeled iCalendar content when updating', function () {
    ...
    $object = DavCalendarObject::factory()->for($calendar, 'calendar')
        ->state(davData(['uid' => 'keep', 'summary' => 'Old']))
        ->create(['calendar_data' => $raw]);
